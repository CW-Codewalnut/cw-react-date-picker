...
export function DatePickerDay({ day, date, dayLabel, isSelectedDate }: any) {
  if (date && day && dayLabel) {
    return (
      <div
        className={clsx(
          // NOTE: A Few styles can be customized from global CSS.
          "!rounded-[4.8px] h-9 w-9 flex flex-col items-center justify-between p-2 !bg-red-400", // Add default `background-color` with important notation for each day in the colander.
          {
            " dark:bg-black": !isSelectedDate,
            "border border-primary dark:border-black !p-[7px] !bg-green-400":
              isSelectedDate, // Add selected day `background-color` with important notation in the colander
          }
        )}
